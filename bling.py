...
class ProductionTimer:
    ...
    def stop_and_log(self, produto_nome):
        ...
        status = self.pause(produto_nome)  # Garante cálculo antes de deletar
    ...
    def _add_to_history(self, registro):
        """Salva no histórico mensal — sempre lê/escreve disco com encoding utf-8."""
        try:
...
class ComponentConsumptionManager:
    ...
    Isso garante persistência mesmo com múltiplos workers Gunicorn no Render.
    """
    ...
    def register_component(self, component_name: str, qty: float, unit: str, product_name: str) -> dict:
        ...
        with self._lock:
            ...
            logger.info(f"✅ Consumo registrado: {component_name} x{qty} ({product_name})")
            return comp

    def unregister_component(self, component_name: str, qty: float, product_name: str):
        """Thread-safe: lê disco → remove último registro → salva disco."""
        with self._lock:
            data = self._load_disk()
            key = self._current_month_key()
            self._ensure_month(data, key)
...
class WebServer:
    ...
    def _setup_routes(self):
        ...
        def api_timers_status():
            """Estado ao vivo de todos os timers + histórico do mês.
            Lê sempre do disco — correto com múltiplos workers Gunicorn."""
    ...
    def _setup_websockets(self):
        ...
        def ws_kpi_updates(ws):
            ...
            try:
                ...
                # Calcula componentes em background e envia para todos
                def _bg_calc():
                    try:
                        usage = self.orchestrator.calculate_component_usage()
                        self.orchestrator._component_usage_cache = usage
...
<body>
    ...
    <script>
        ...
        // Via WebSocket broadcast — usa API dedicada para vendas (mais precisa)
        ...
        async function refreshComponentTab() {
            // 1. Insumos registrados (checklist — lido do disco)
            try {
            ...
            // 2. Timers ativos + histórico (rota dedicada, sempre do disco)
            try {
            ...
            } catch(e) { console.error('Erro timers:', e); }
            // 3. Produtos vendidos (qtd real por item do pedido)
            try {
                const salesData = await fetchAPI('/api/sales/monthly-products');
                renderMonthlySales(salesData.produtos || [], salesData.mes || '');
        ...
        function renderActiveTimers(activeProduction) {
            ...
            const rows = activeProduction.map((p, i) => {
                ...
                return '<tr>' +
                    ...
                    '<td class="text-center font-monospace fw-bold fs-5 text-primary" id="lt_' + i + '">' +
                        formatSeconds(p.tempo_decorrido) + '</td>' +
                    ...
                    '<td class="text-center"><button class="btn btn-sm btn-outline-primary" ' +
                        'onclick="openProductionChecklist(\'' + safe + '\')">Abrir Timer</button></td>' +
                '</tr>';
            }).join('');
            ...
            // Inicia contador local para cada timer em execução
        ...
        function renderMonthlySales(produtos, mes) {
            ...
            if (!div) return;
            // Aceita lista [{nome,qtd}] (nova API) ou dict legado {nome:qtd}
            ...
            const hdr = mesLabel
                ? '<div class="px-3 py-2 border-bottom bg-light"><small class="text-muted">📅 ' + mesLabel +
                  ' • ' + entries.length + ' produto(s)</small></div>'
                : '';
        ...
        function setupKpiWebSocket() {
            ...
            wsKpi.onclose = () => {
                console.log("WebSocket KPI desconectado. Reconectando em", _wsDelay, "ms...");
                setTimeout(() => {
                    const proto = window.location.protocol === 'https:' ? 'wss' : 'ws';
        ...
        // Watchdog 8s: se WS não responder, faz fallback HTTP
        // Garante que "CARREGANDO" nunca fica travado
        let _wsGotMsg = false;
        ...
        wsKpi.addEventListener('message', () => {
            if (!_wsGotMsg) { _wsGotMsg = true; clearTimeout(_wsWatchdog); }
        });
