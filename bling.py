#!/usr/bin/env python3 ““” bling_enhanced.py - Sistema completo de
automação Bling com: - Autenticação automática persistente - Logs em
tempo real via WebSocket - Interface web sem erros - Configuração
automática de componentes ““”

import os import sys import json import time import logging import
argparse import base64 from pathlib import Path from datetime import
datetime, timedelta from typing import Dict, List, Optional, Tuple from
dataclasses import dataclass, asdict from urllib.parse import urlencode
from collections import defaultdict from threading import Lock, Thread

import requests from flask import Flask, request,
render_template_string, jsonify, redirect, url_for from dotenv import
load_dotenv

Tenta importar flask_sock, mas funciona sem ele

try: from flask_sock import Sock WEBSOCKET_AVAILABLE = True except
ImportError: WEBSOCKET_AVAILABLE = False Sock = None
...
COLORS_ENABLED = True except ImportError: class Fore: GREEN = RED =
YELLOW = CYAN = MAGENTA = BLUE = RESET = ’’ class Style: BRIGHT =
RESET_ALL = ’’ COLORS_ENABLED = False
...
class InMemoryLogHandler(logging.Handler): def init(self, max_logs=500):
super().__init__() self.logs = [] self.max_logs = max_logs self.lock =
Lock()
...
memory_handler.setFormatter(logging.Formatter(‘%(asctime)s - %(name)s -
%(levelname)s - %(message)s’))

logging.basicConfig( level=logging.INFO, format=‘%(asctime)s -
%(name)s - %(levelname)s - %(message)s’, handlers=[
logging.FileHandler(‘logs/automacao_bling.log’, encoding=‘utf-8’),
logging.StreamHandler(sys.stdout), memory_handler ] ) logger =
logging.getLogger(name)

error_logger = logging.getLogger(‘errors’) error_handler =
logging.FileHandler(‘logs/errors.log’, encoding=‘utf-8’)
error_handler.setLevel(logging.ERROR)
error_handler.setFormatter(logging.Formatter(‘%(asctime)s -
%(levelname)s - %(message)s’)) error_logger.addHandler(error_handler)
...
class Config: “““Configurações globais”“” CLIENT_ID =
os.getenv(‘BLING_CLIENT_ID’, ’‘) CLIENT_SECRET =
os.getenv(’BLING_CLIENT_SECRET’, ’‘) REDIRECT_URI =
os.getenv(’BLING_REDIRECT_URI’,
‘https://bling-automacao.onrender.com/callback’)
...
def print_success(msg: str): print(f”{Fore.GREEN}✓
{msg}{Style.RESET_ALL}“)

def print_error(msg: str): print(f”{Fore.RED}✗ {msg}{Style.RESET_ALL}“)

def print_warning(msg: str): print(f”{Fore.YELLOW}⚠
{msg}{Style.RESET_ALL}“)

def print_info(msg: str): print(f”{Fore.CYAN}ℹ {msg}{Style.RESET_ALL}“)
...
@dataclass class Component: sku: str name: str qty: int supplier: str
lead_time_days: int unit_cost: float = 0.0 min_stock: int = 10
current_stock: int = 0

@dataclass class Kit: sku: str name: str components: List[Component]
price: float = 0.0

@dataclass class PurchaseNeed: component_sku: str component_name: str
quantity_needed: int supplier: str lead_time_days: int reason: str
...
class BlingAuth: TOKEN_FILE = ‘tokens.json’
...
class BlingAPI: BASE_URL = ‘https://www.bling.com.br/Api/v3’
...
class PurchaseNeedsManager: def init(self, api: BlingAPI): self.api =
api self.needs: Dict[str, PurchaseNeed] = {}
...
class StatisticsManager: def init(self): self.reset()
...
class AutomationOrchestrator: COMPONENT_CONFIG_FILE =
...
class WebServer: def init(self, auth: BlingAuth, orchestrator:
AutomationOrchestrator): self.app = Flask(name)
...
def run_cli(): Path(“logs”).mkdir(exist_ok=True)

parser = argparse.ArgumentParser( description=‘Automação Bling
Enhanced - Sistema completo de automação ERP’,
formatter_class=argparse.RawDescriptionHelpFormatter, epilog=““”
Exemplos de uso: python bling_enhanced.py –serve Inicia o servidor web
(padrão) python bling_enhanced.py –run Executa processamento de kits
python bling_enhanced.py –serve –port 5000 Servidor em porta customizada
““” ) parser.add_argument(‘–serve’, action=‘store_true’, help=‘Inicia
servidor web’) parser.add_argument(‘–run’, action=‘store_true’,
help=‘Processa kits e componentes’) parser.add_argument(‘–port’,
type=int, default=None, help=‘Porta do servidor (padrão: 8000 ou PORT
env)’)

args = parser.parse_args()

if not args.serve and not args.run: args.serve = True

config = Config()

if not config.CLIENT_ID or not config.CLIENT_SECRET: print_error(“✗
BLING_CLIENT_ID e/ou BLING_CLIENT_SECRET não definidos. Configure as ENV
vars no Render.”) sys.exit(1)

if not config.REDIRECT_URI: print_error(“✗ BLING_REDIRECT_URI não
definido. Defina para ‘https://.onrender.com/callback’ no Render.”)
sys.exit(1)

if args.serve: print_header(“BLING AUTOMAÇÃO - MODO SERVIDOR”)
...
def create_app(): # Replicando a lógica de configuração necessária para
o WebServer config = Config() auth = BlingAuth(config) orchestrator =
AutomationOrchestrator(config)

# A instância do Flask está em server.app server = WebServer(auth,
orchestrator)

# O Gunicorn não deve lidar com threads de background. # A lógica de
carregamento inicial deve ser refeita para ser síncrona # ou o Gunicorn
deve ser configurado para usar workers de thread. # Por enquanto, vamos
retornar o app.
